...
export default function MixerPanel({ mixer, available, onChange, onDownload, onContext }: {
  ...
  onContext: (e: React.MouseEvent, key: TrackName) => void;
}) {
  return <div className="mixer-panel"><div className="section-label">AUDIO MIXER</div>{(['original', 'vocals', 'instrumental'] as const).map(key => {
    ...
    return <div key={key} onContextMenu={e=>onContext(e,key)} className={'mixer-track ' + (audible && ready ? 'audible' : '')}>
      <div className="mixer-title"><Music2 size={14}/><strong>{state.name || trackNames[key]}{state.enabled === false ? ' · Disabled' : ''}</strong><span className="mixer-dot" title={ready ? audible ? 'Audible' : 'Silent' : 'Separate to enable'}/>{key !== 'original' && <button aria-label={`Download ${key}`} disabled={!ready} onClick={() => onDownload(key)}><Download size={13}/></button>}</div>
